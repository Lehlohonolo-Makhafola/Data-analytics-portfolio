...
dealership-sales/
│
├── Dealership\_sales\_\_Raw\_.csv          # Source data (unmodified)
├── Dealership\_sales\_project.xlsx       # Cleaned workbook with 4 sheets:
...
│
├── Dealership\_sales\_dashboard.png      # Dashboard screenshot
└── README.md
```
---
...
Duplicate rows	0
Columns in raw file:
`ORDERNUMBER` · `QUANTITYORDERED` · `PRICEEACH` · `ORDERLINENUMBER` · `SALES` · `ORDERDATE` · `STATUS` · `QTR\_ID` · `MONTH\_ID` · `YEAR\_ID` · `PRODUCTLINE` · `MSRP` · `PRODUCTCODE` · `CUSTOMERNAME` · `PHONE` · `ADDRESSLINE1` · `ADDRESSLINE2` · `CITY` · `STATE` · `POSTALCODE` · `COUNTRY` · `TERRITORY` · `CONTACTLASTNAME` · `CONTACTFIRSTNAME` · `DEALSIZE`
---
🛠️ Data Issues Identified & Resolved
Issue 1 — Incorrect Revenue Values (HIGH IMPACT)
Column: `SALES` → renamed `Revenue`
The `SALES` column contained incorrect values in 1,304 out of 2,823 rows (46.2%) — a critical data integrity failure. The correct revenue for each line item is `QUANTITY\_ORDERED × PRICE\_EACH`. The original values did not match this formula and would have produced a materially incorrect total revenue figure had they been used for reporting.
Metric	Value
Rows affected	1,304 (46.2%)
Resolution	Recalculated as `QUANTITY\_ORDERED × PRICE\_EACH`
...
Column: `ORDERDATE` → renamed `ORDER\_DATE`
...
Column: `CUSTOMERNAME` → renamed `CUSTOMER\_NAME`
...
---
Issue 5 — Blank Cells in `ADDRESS\_LINE\_2` and `STATE` (LOW IMPACT)
Columns: `ADDRESSLINE2` → `ADDRESS\_LINE\_2` | `STATE`
...
All 25 column names were renamed from inconsistent concatenated ALLCAPS to consistent `UPPER\_SNAKE\_CASE`, improving readability and compatibility with SQL and BI tools.
...
Dealership sales	Revenue	Incorrect/inconsistent sales values in 1,304 rows	YES	Recalculated as `QUANTITY\_ORDERED × PRICE\_EACH`	High
...
---
Dataset source: Public sample sales dataset. All a
